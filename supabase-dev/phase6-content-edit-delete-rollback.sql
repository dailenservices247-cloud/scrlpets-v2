-- Roll back Phase 6 content edit/delete.
-- WARNING: hard-deleted post rows cannot be restored by this rollback.

drop view public.unified_feed;

-- Restore the phase5d feed contract.
...
drop trigger posts_touch_updated_at on public.posts;
drop trigger listings_touch_updated_at on public.listings;
drop trigger posts_identity_immutable on public.posts;
drop trigger listings_identity_immutable on public.listings;
...
alter table public.listings drop column deleted_at;
