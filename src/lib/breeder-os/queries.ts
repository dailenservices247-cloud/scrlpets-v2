...
export async function getRoster(ownerId: string): Promise<RosterAnimal[]> {
  ...
  const { data: creatures } = await supabase
    ...
    .eq("owner_id", ownerId)
    // A recorded ancestor is not roster stock: it cannot be attested, listed,
    // or counted toward readiness. Without this, recording one pedigree marks
    // the "add your animals" onboarding step done.
    .eq("in_roster", true)
    .is("archived_at", null)
